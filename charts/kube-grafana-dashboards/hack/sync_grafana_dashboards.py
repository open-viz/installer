...
charts = [
    {
        ...
        'multicluster_key': '.Values.dashboard.multicluster.global.enabled',
    },
    {
        ...
        'type': 'json',
        'min_kubernetes': '1.14.0-0',
        'multicluster_key': '(or .Values.dashboard.multicluster.global.enabled .Values.dashboard.multicluster.etcd.enabled)',
    },
    # CoreDNS
    # https://gist.github.com/tamalsaha/9c7e36cd95d45f9491fbf711e457153a
    {
        'source': 'https://gist.githubusercontent.com/tamalsaha/9c7e36cd95d45f9491fbf711e457153a/raw/25009fb4d1cbfc26a053bf453038b01a8eaffcda/k8s-coredns.json',
        'destination': '../templates/grafana/dashboards-1.14',
        'type': 'json',
        'min_kubernetes': '1.14.0-0',
        'multicluster_key': '.Values.dashboard.multicluster.global.enabled',
...
condition_map = {
    ...
    'prometheus-remote-write': ' .Values.prometheus.remoteWriteDashboards',
}
...
{{- if and (semverCompare ">=%(min_kubernetes)s" $kubeTargetVersion) (semverCompare "<%(max_kubernetes)s" $kubeTargetVersion) %(condition)s }}
apiVersion: openviz.dev/v1alpha1
kind: GrafanaDashboard
metadata:
  name: {{ printf "%%s-%%s" (include "kube-grafana-dashboards.fullname" $) "%(name)s" | trunc 63 | trimSuffix "-" }}
...
def patch_dashboards_json(content):
    try:
        ...
        # remove id, uid
        content_struct.pop('id', None)
        content_struct.pop('uid', None)
        ...
        content = json.dumps(content_struct, indent=4)
...
def patch_json_set_multicluster_as_variable(content, multicluster_key):
    # content is no more in json format, so we have to replace using regex
    return re.sub(r"':multicluster:'", '{{ if %s }}0{{ else }}2{{ end }}' % multicluster_key, content, flags=re.IGNORECASE)


def patch_json_set_timezone_as_variable(content):
    # content is no more in json format, so we have to replace using regex
    return re.sub(r'timezone\s*:.*', "timezone: '{{ .Values.grafana.defaultDashboardsTimezone }}'", content, flags=re.IGNORECASE)
...
def write_group_to_file(resource_name, content, url, destination, min_kubernetes, max_kubernetes, multicluster_key):
    ...
    filename_struct = {'model': yaml.full_load(content)}
    model = yaml_str_repr(filename_struct)

    # rules themselves

    model = patch_json_set_multicluster_as_variable(model, multicluster_key)
    model = patch_json_set_timezone_as_variable(model)
