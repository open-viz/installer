...
[Grafana Operator by AppsCode](https://github.com/searchlight/grafana-operator)
...
This chart bootstraps a [HashiCorp Vault controller](https://github.com/searchlight/grafana-operator) deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
```console
$ helm uninstall grafana-operator -n kube-system
```
...
| `operator.repository`                   | Vault operator container image                                                                                                                                                                                                                                                                                                                              | `grafana-operator`                                          |
...
| `apiserver.servingCerts.generate`       | If true, generate on install/upgrade the certs that allow the kube-apiserver (and potentially ServiceMonitor) to authenticate grafana-operator pods. Otherwise specify in `apiserver.servingCerts.{caCrt, serverCrt, serverKey}`. See also: [example terraform](https://github.com/searchlight/grafana-operator/blob/master/charts/grafana-operator/example-terraform.tf) | `true`                                                    |
...
```console
$ helm install grafana-operator appscode/grafana-operator -n kube-system --set image.tag=v0.3.0
```
